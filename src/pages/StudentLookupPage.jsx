    ...
    <div className="space-y-stack-lg max-w-2xl mx-auto">
        ...
        <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-surface-container border border-white/5 text-on-surface text-sm rounded-lg pl-10 pr-4 py-2.5 outline-none focus:border-primary font-label-bold"
            />
          </div>
      ...
      {studentResult && (
        <div className="glass-card p-stack-md rounded-xl w-full flex flex-col gap-4 border-primary/40 active-glow">
          ...
          <div className="bg-surface-container-low p-3.5 rounded-xl border border-white/5 text-xs text-on-surface-variant flex items-center gap-2">
      ...
      <div className="glass-card p-4 rounded-xl w-full flex flex-wrap items-center gap-2 text-xs text-on-surface-variant">
        ...
        {studentRegistry.map((s) => (
          <button
            ...
            className="px-2.5 py-1 rounded-lg bg-surface-container border border-white/5 text-primary hover:bg-primary/10 cursor-pointer font-label-bold text-xs transition-colors"
          >
            {s.regNo} ({s.labGroup})
          </button>
        ))}
